...
$temp = $PERMISSIONS['Edit_Settings']+$PERMISSIONS['Edit_Styles']+$PERMISSIONS['Edit_Permissions']+$PERMISSIONS['Edit_Users']+$PERMISSIONS['Edit_Room']+$PERMISSIONS['Server_Ban']+$PERMISSIONS['Edit_Filter']+$PERMISSIONS['Edit_Smilies']+$PERMISSIONS['Edit
_Bandwidth'];
if($temp == 0)
	$accessd = 0;
...
<tr>
<td width=\"200\"><b>Uívate¾</b></td>
<td width=\"100\"><b>Expiruje za..</b></td>
<td width=\"250\"><b>Akcia</b></td>
</tr>";
...
<td width=\"250\">
<a href=\"admin.php?action=editusers&subaction=delete&id=$row[0]\">[Zma]</a> </a>
<a href=\"admin.php?action=permissions&workon=$row[1]\">[Oprávnenia]</a> ";
if($row[4] == 5 || $row[4] == 4){
$body .= "<a href=\"admin.php?action=editusers&subaction=deadmin&id=$row[0]\">[Vezmi Admina]";
}else{
$body .= "<a href=\"admin.php?action=editusers&subaction=admin&id=$row[0]\">[Daj Admina]";
}
$body .="</a> ";
if($row[12] == 0){
$body .= "<a href=\"admin.php?action=editusers&subaction=expire&id=$row[0]\">[Expiruje]";
}else{
$body .= "<a href=\"admin.php?action=editusers&subaction=nexpire&id=$row[0]\">[Neexpiruje]";
}
$body .= "</a> ";
$q2 = DoQuery("SELECT * FROM $SERVER[TBL_PREFIX]bans WHERE room='*' AND name='$row[1]'");
$row2 = Do_Fetch_row($q2);
if($row2[0] == ""){
$body .= "<a href=\"admin.php?action=editusers&subaction=ban&user=$row[1]\">[Zabanuj]";
}else{
$body .= "<a href=\"admin.php?action=editusers&subaction=unban&user=$row[1]\">[Odbanuj]";
}
$body .="</a></td>
</tr>";
}
...
$body .= "<table border=\"1\" cellspacing=\"0\" cellpadding=\"0\">
<tr>
<td width=\"200\"><b>Názov</b></td>
<td width=\"100\"><b>Expiruje za..</b></td>
<td width=\"100\"><b>Typ</b></td>
<td width=\"100\"><b>Heslo</b></td>
<td width=\"150\"><b>Akcia</b></td>
</tr>";
...
$body .= "</td>
<td width=\"150\">
<a href=\"admin.php?action=editrooms&subaction=delete&id=$row[0]\">[Zma]</a> ";
if($row[9] == 0){
$body .= "<a href=\"admin.php?action=editrooms&subaction=expire&id=$row[0]\">[Expiruje]";
}else{
$body .= "<a href=\"admin.php?action=editrooms&subaction=nexpire&id=$row[0]\">[Neexpiruje]";
}
$body .= "</a> </td>
</tr>";
}
...
$row = Do_Fetch_Row($q);
if($row[0] == "")
	@DoQuery("INSERT INTO $SERVER[TBL_PREFIX]permissions VALUES('0','$workon','$createrooms','$neverexpirerooms','$privaterooms','$moderated','$adminpowers','$oppowersown','$oppowersany','$iplookup','$kick','$ban','$banserver','$sysmessage','$edit_se
ttings','$edit_styles','$edit_permissions','$edit_users','$edit_rooms','$edit_smilies','$edit_filter','$edit_bandwidth')");
else
	@DoQuery("UPDATE $SERVER[TBL_PREFIX]permissions SET CreateRoom='$createrooms',CR_NeverExpire='$neverexpirerooms',CR_Private='$privaterooms',CR_Moderated='$moderated',Make_Admins='$adminpowers',Give_Ops_Own='$oppowersown',Give_Ops_All='$oppowersan
y',Lookup_Ips='$iplookup',Kick='$kick',Ban='$ban',Server_Ban='$banserver',Send_Sys_Message='$sysmessage',Edit_Settings='$edit_settings',Edit_Styles='$edit_styles',Edit_Permissions='$edit_permissions',Edit_Users='$edit_users',Edit_Room='$edit_rooms' ,Edit
_Smilies='$edit_smilies',Edit_Filter='$edit_filter',Edit_Bandwidth='$edit_bandwidth' WHERE user='$workon'");
