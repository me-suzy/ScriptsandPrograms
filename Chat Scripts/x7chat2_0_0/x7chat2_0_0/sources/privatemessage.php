...
?><?PHP
	...
	function pm_base_frame(){
		...
		<html>
		<head>
		<title><?=$x7c->settings['site_name']?></title>
			<?PHP
				// Echo the styles
			...
			<script langauge="javascript" type="text/javascript">
				...
				function msgSent(){
					...
					if(message != ""){
						...
						// Put it into screen
						window.frames['main_window'].document.write('<span class="you"><?=$x7s->username?>'+timestamp+':</span> '+message+'<Br>');
			...
			<table border="0" width="428" cellspacing="1" cellpadding="1" class="pm_infobar">
				<tr>
					<td width="128" class="pm_ib_fc"><?=$_GET['send_to']?></td>
					<td width="75" class="pm_ib_r" onMouseOut="javascript: this.className='pm_ib_r';" onMouseOver="javascript: this.className='pm_ib_r_alt';" onClick="javascript: window.open('./index.php?act=view_profile&user=<?=$_GET['send_to']?>','','location=no,menubar=no,resizable=no,status=no,toolbar=no,scrollbars=yes,width=<?=$x7c->settings['tweak_window_large_width']?>,height=<?=$x7c->settings['tweak_window_large_height']?>');"><?=$txt[87]?></td>
					<td width="75" class="pm_ib_r" onMouseOut="javascript: this.className='pm_ib_r';" onMouseOver="javascript: this.className='pm_ib_r_alt';" onClick="javascript: window.open('./index.php?act=usercp&cp_page=msgcenter&to=<?=$_GET['send_to']?>','','location=no,menubar=no,resizable=no,status=no,toolbar=no,scrollbars=yes,width=<?=$x7c->settings['tweak_window_large_width']?>,height=<?=$x7c->settings['tweak_window_large_height']?>');"><?=$txt[89]?></td>
					<td width="75" class="pm_ib_r" onMouseOut="javascript: this.className='pm_ib_r';" onMouseOver="javascript: this.className='pm_ib_r_alt';" onClick="javascript: window.open('./index.php?act=usr_action&action=ignore&user=<?=$_GET['send_to']?>&room=','','location=no,menubar=no,resizable=no,status=no,toolbar=no,scrollbars=yes,width=<?=$x7c->settings['tweak_window_small_width']?>,height=<?=$x7c->settings['tweak_window_small_height']?>');window.close();"><?=$txt[91]?></td>
					<td width="75" class="pm_ib_r" onMouseOut="javascript: this.className='pm_ib_r';" onMouseOver="javascript: this.className='pm_ib_r_alt';" onClick="javascript: window.close();"><?=$txt[133]?></td>
				</tr>
			</table>
			
			<Br>
			<iframe name="send" style="visibility: hidden;" width="0" height="0" src="./index.php?act=pm&pmf=send&send_to=<?=$_GET['send_to']?>"></iframe>
			<iframe name="update" style="visibility: hidden;" width="0" height="0" src="./index.php?act=pm&pmf=update&send_to=<?=$_GET['send_to']?><?=$extra?>"></iframe>
			...
			<input type="hidden" name="send_to" value="<?=$_GET['send_to']?>">
					...
					<td width="95">
						<!-- Begin Select Area for Font -->
							<table border="0" cellspacing="0" cellpadding="0" class="nonSelected" onMouseOver="javascript: clearTimeout(fontTimeout);" onMouseOut="javascript: fontTimeout = setTimeout('closeMenu(\'fontselector\');',750);">
								<tr valign="middle">
									<td height="17" class="selectbar" onClick="javascript: ClickedSelector('fontselector');" onMouseOver="javascript: document.font_selector.src='<?=$print->image_path?>selectarrow_over.gif'" onMouseOut="javascript: document.font_selector.src='<?=$print->image_path?>selectarrow.gif'">&nbsp;<input type="text" readonly="true" name="curfont" value="<?=$x7c->settings['default_font']?>" class="curfont"></td>
					...
					<td width="75">
						<!-- Begin Select Area for Size -->
							<table border="0" cellspacing="0" cellpadding="0" class="nonSelected"   onMouseOver="javascript: clearTimeout(sizeTimeout);" onMouseOut="javascript: sizeTimeout = setTimeout('closeMenu(\'sizeselector\');',750);">
								<tr valign="middle">
									<td height="17" class="selectbar" onClick="javascript: ClickedSelector('sizeselector');" onMouseOver="javascript: document.size_selector.src='<?=$print->image_path?>selectarrow_over.gif'" onMouseOut="javascript: document.size_selector.src='<?=$print->image_path?>selectarrow.gif'">&nbsp;<input type="text" readonly="true" name="cursize" value="<?=$x7c->settings['default_size']?>" class="cursize"></td>
						...
						<div style="position: relative; top: -50px;z-index: 2;">
							<table border="0" cellspacing="0" cellpadding="0">
								<tr valign="middle">
									...
									<td width="5">&nbsp;</td>
									<!--<td><input type="image" src="<?=$print->image_path?>send.gif" onClick="javascript: msgSent();" style="cursor: pointer; cursor: hand;" onMouseOut="this.src='<?=$print->image_path?>send.gif'" onMouseOver="this.src='<?=$print->image_path?>send_over.gif'"></td>-->
									<td><input type="submit"  onClick="javascript: msgSent();" style="cursor: pointer; cursor: hand;background: url(<?=$print->image_path?>send.gif);border: none;height: 25px;width: 55px;text-align: center;font-weight: bold;" onMouseOut="this.style.background='url(<?=$print->image_path?>send.gif)'" onMouseOver="this.style.background='url(<?=$print->image_path?>send_over.gif)'" value="<?=$txt[181]?>"></td>
								</tr>
							</table>
						</div>
					</td>
