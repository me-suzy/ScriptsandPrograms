...
?><?PHP
	...
	class session {
		...
		function session(){
			global $X7CHAT_CONFIG,$db,$auth_ucookie,$auth_pcookie;
		...
		function dologin(){
			global $X7CHAT_CONFIG,$db,$auth_ucookie,$auth_pcookie,$x7c,$x7s,$prefix,$g_default_settings,$remove_old_guest_logs,$txt;
			...
			if($temp->loggedin == 1){
				...
				if($x7c->settings['allow_guests'] == 1){
					...
					if($row[0] == ""){
						...
						$db->DoQuery("INSERT INTO {$prefix}users (id,username,password,status,user_group,time,settings,hideemail,ip) VALUES('0','$_POST[username]','$_POST[password]','$txt[150]','{$x7c->settings['usergroup_guest']}','$time','{$g_default_settings}','0','$ip')");
