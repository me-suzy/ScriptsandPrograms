...
?><?PHP
	...
	function return_last_time($room){
		...
		// Get current data from the online table
		$query = $db->DoQuery("SELECT * FROM {$prefix}online WHERE name='$x7s->username' AND room='$room'");
		$row = $db->Do_Fetch_Row($query);
		$time = time();
		
		// See if their online list needs updated
		$user_online = get_online($room);
		$total_us = $user_online;
		$user_online = implode(",",$user_online);
		$back_2_db = $user_online;
		
		$return[0] = array();
		
		if($row[4] != $user_online){
			// It does, see which username's have entered/left
			$server_side_vals = explode(",",$user_online);
			$client_side_vals = explode(",",$row[4]);
			
			// Get the names of the people who entered

			$master_array = array_diff($server_side_vals,$client_side_vals);
			foreach($master_array as $key=>$val){
				$return[0][] = array("0"=>"$val","1"=>"1");
			}
			
			// Get the names of the people who left
			$master_array = array_diff($client_side_vals,$server_side_vals);
			foreach($master_array as $key=>$val){
				$return[0][] = array("0"=>"$val","1"=>"0");
			}
		}
		
		// User is entering room for the first time
		if($row[0] == ""){
			// See if the room is full
			if(count($total_us) >= $x7c->room_data['maxusers'])
				return 2;
		...
		// Set this to the last time the user got messages
		$return[1] = $row[5];
	...
	*/
	...
	function get_online($room="*"){
		...
		if(!isset($querydb['get_online_1'])){
			...
			$exp_time = time() - $x7c->settings['online_time'];
			$query = $db->DoQuery("SELECT name,room FROM {$prefix}online WHERE time>'$exp_time'{$invis}ORDER by name ASC");
