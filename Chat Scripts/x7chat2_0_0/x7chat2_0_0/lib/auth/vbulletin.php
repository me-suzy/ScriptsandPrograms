...
?><?PHP
	...
	if (isset ($_COOKIE[$cookieprefix.'userid']) && isset($_COOKIE[$cookieprefix.'password'])) {
		...
		$result = $vbc->DoQuery("SELECT userid, username, usergroupid, password, salt FROM {$tableprefix}user WHERE userid='".$_COOKIE[$cookieprefix.'userid']."'");
	...
	function vb_init($username){
		...
		$result = $vbc->DoQuery("SELECT userid, username, password, salt FROM {$tableprefix}user WHERE username='".$username."'");
		$member = $vbc->Do_Fetch_Row($result);
		return $member[3];
	}
	...
	function auth_getpass($auth_ucookie){
		...
		$q = $vbc->DoQuery("SELECT password, usergroupid FROM {$tableprefix}user WHERE username='$_COOKIE[$auth_ucookie]'");
