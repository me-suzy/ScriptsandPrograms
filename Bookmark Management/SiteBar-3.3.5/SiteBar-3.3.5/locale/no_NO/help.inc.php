...
$help['103'] = <<<_P
<@><p><strong>Søk</strong> - Tillater søk i alle viste lenker. Det er mulig å
spesifisere søket ved bruk av prefiksene <strong>url:</strong>,
<strong>navn:</strong>, <strong>beskr:</strong>, <strong>alle:</strong>.
Standard prefiks er <strong>navn:</strong> og kan endres i "Brukerinstillinger".
Når matchende lenker eller mapper blir funnet, blir de markert og et javascript
bekreftelsesvindu vises noen av detaljene. Brukeren har muligheten til å
fortsette søket eller stoppe det.

<p><strong>Markér</strong> - Samme som søk, men uten javascript-melding.

<p><strong>Skjul alle</strong> - Skjuler alle noder. Når du klikker på den
for andre gang (når alle noder er skjult allerede) så utvides alle noder.
...
funksjonen er her fordi pluginen kan befinne seg i et sidepanel (avhengig
av nettleser) slik at brukeren ellers ikke har mulighet til å oppdatere den.
_P;
...
med bookmarklet lar den brukeren velge målmappe. Ellers opprettes den mappen du
klikket på.
...
$help['303'] = <<<_P
I SiteBar kan brukere designe drakter. Til det kreves det god kjennskap til 
CSS, samt tilltrekkelig kkjennskap til XSLT for å kunne tilpasse designet.
For å lage en ny drakt, bruk en eksisterene drakt. Kopier en eksiterende 
drakt fra mappa "skins" og gi den et passende navn. Hver drakt består av 
<ul><li>Flere bilder (Bildene må være i formatet PNG)
<li>Hook-fila "hook.inc.php". Brukes for bla for å få info om drakta (fx. forfatternavn). I hook-fila er det mulig å endre hodet og fot for sitebar-installasjonen.
<li>Felles stilsett "common.css" for fargedefinisjon for alle drakter.
<li>Stilsettet for SiteBar-panelet "sitebar.css".
<li>For XSLT-baserte skriventer er det stilsett for visning av lenkenytt
"news.css", for emnekatalogen "directory.css" og for backendsøk "search.css".
</ul>

<strong>XSL</strong> - det er mulig å endre presentasjonen fullstendig på XML-baserte 
SiteBar-outputs vha. ditt eget XSL-stilsett. I såfall må du kopiere fila skins/*.xsl.php fra 
en av de eksisterende draktene til din draktmappe og gjøre dine endringer der. <p> 

<strong>Cascading</strong> - med unntak av det felles stilsettet, er alle andre stilsett laget
som en undergruppe av det.<p>
<strong>Branding</strong> - for de som ønsker å lage sin egen drakt tilpasset sitt eget nettsteds design, er det anbefalt å fjerne alle andre drakter og å velge "default skin" i SiteBars innstillinger.
<p>Hvis du skulle ønske å inkludere din drakt til SiteBars distribusjon, må
du kontakte utviklingsteamet og teste din drakt med den siste stabile utviklerversjon. 
Som en regler må logoene for SiteBar og SourceForge befinne seg å¨siden, men SiteBar-logoen
kan du fritt tilpasse.
_P;

$help['305'] = <<<_P
For å kunne foreta en migrering av en eksisterene SiteBar-installasjon til en annen
tjener er følgende påkrevet
<ul>
    <li>Exporter tabellene sitebar_* fra databasen til en .SQL-fil.
    <li>Importer denne fila til den nye databasen.
    <li>Flytt eksisterende installasjon eller installere en ny stabil SiteBar-versjon
        (nedgradering og oppgradering vil skje automatisk).
    <li>Dersom detaljene for den nye databaseforbindelsen er endret, sletter du eller
        oppdater dem i fila inc/config.inc.php,

</ul>

<p>
Eksport og import kan du gjøre ved hjelp av <a href='http://www.phpmyadmin.net/'>phpMyAdmin</a>.
Tabellen sitebar_favicon (til 3.2.6) eller sitebar_cache (fra og med 3.3) behøver du ikke å
overføre, Deres innhold vil bli bygget opp på nytt.
_P;
