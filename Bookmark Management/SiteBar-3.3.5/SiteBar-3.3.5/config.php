...
Use any of the following to create file <strng>config.inc.php</strong>
...
<?php
    ...
    function conversion($from, $upgrade=true)
    {
        ...
        while ($from!=$this->db->currentRelease() &&
    ...
    function install()
    {
        ...
        if (!$this->hasErrors())
        {
            ...
            $bm->loadFile('sql/bookmarks_sitebar.xml', 'xbel');

            if ($bm->success)
            {
                $tree =& SB_Tree::staticInstance();
                $tree->importTree(2, $bm->root, false);
            }

            $bm = new SB_Loader(false);
            $bm->loadFile('sql/bookmarks_websearch.xml', 'xbel');
