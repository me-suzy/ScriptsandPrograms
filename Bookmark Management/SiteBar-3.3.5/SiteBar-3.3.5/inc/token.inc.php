...
class SB_Token extends SB_ErrorHandler
{
    ...
    function createToken($type, $uid, $expires)
    {
        ...
        return SB_Page::baseurl().'/token.php?'.$uid.'='.$token;
    }
    ...
    /**
    * If the token is invalid, then all tokens for the given email will be invalidated.
    */
    function validate($uid, $token, $redirect=false)
    {
        ...
        if ($tokenRec)
        {
            ...
            case 'verify':
                ...
                $this->um->autoJoinGroups($user['uid'], $user['email'], true);
                ...
                if ($this->um->getParam('config', 'users_must_verify_email'))
                {
                    ...
                    $paraAtt = array($user['name'],$user['email'],SB_Page::baseurl());

                    if ($this->um->getParam('config', 'users_must_be_approved') && !$user['approved'])
                    {
                        $paraName = 'usermanager::signup_approval_verified';
                        $paraAtt[] = $this->um->getApproveUserUrl($user['email']);
                        $paraAtt[] = $this->um->getRejectUserUrl($user['email']);
                ...
                if ($redirect)
                {
                    header('Location: command.php?command=Email+Verified&do=yes&uid='.$uid);
                }
                break;
            ...
            case 'reset':
                if ($redirect)
                {
                    header('Location: command.php?command=New+Password&uid='.$uid.'&token='.$token);
                }
                break;
            }
        ...
        if ($redirect)
        {
            header('Location: command.php?command=Invalid+Token&do=yes');
        }
