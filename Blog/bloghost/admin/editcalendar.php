...
if($getadmin3['status']>=1)
{
   ...
   if(isset($_POST['submit']))
   {
      ...
      if($val==1)
      {
         $editcal="update bl_calender set datecotent='$reminders', viewable='$view' where dateclass='$ID' and personid='$getadmin3[adminid]'";
         mysql_query($editcal) or die("Could not edit calendar");
         print "Calendar updated";
      }
      else
      {
         $createevent="Insert into bl_calender (datecotent,viewable,dateclass, personid) values ('$reminders','$view','$ID','$getadmin3[adminid]')";
         mysql_query($createevent) or die(mysql_error());
         print "Entry updated.";
      }
      print "Back to <A href='index.php'>Main</a>";

   }
   else
   {
      $ID=$_GET['ID']; 
      $getcontents="SELECT * from bl_calender where dateclass='$ID' and personid='$getadmin3[adminid]'";
