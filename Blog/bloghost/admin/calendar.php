...
{
  $getinfo="SELECT * from bl_calender where personid='$getadmin3[adminid]'";
  $getinfo2=mysql_query($getinfo) or die(mysql_error());
       ...
       { 
         if($i < $offset_count)
           ...
           { 
             ...
             $day_link = "<A href='editcalendar.php?ID=$datecount'>$d</a>";
             if($date == mktime(0,0,0,$month,$d,$year))
             { 
               echo "<td width=14%><A href='editcalendar.php?ID=$datecount'><font color='red'><b>$d</b></font></a><br>";
               while($getinfo3=mysql_fetch_array($getinfo2))
               {
                 if($getinfo3[dateclass]==$datecount)
                 {
                    print "$getinfo3[datecotent]";
                 }
               }
               mysql_data_seek($getinfo2,0);
               print "</td>"; 
             } 
             else 
             { 
               echo "<td class=\"days\" width=14%>$day_link<br>";
               while($getinfo3=mysql_fetch_array($getinfo2))
               {
                 if($getinfo3[dateclass]==$datecount)
                 {
