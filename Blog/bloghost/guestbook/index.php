...
else
{
  ...
  $order="SELECT * from bl_gbook where identifier='$getvars3[adminid]'";
  ...
  if($start>=$numentries)
  {
    print "<A href='index.php?membername=$membername&start=$prev'><<</a>&nbsp;";
  }
  while($order3=mysql_fetch_array($order2))
  {
     if($f>=$start-3*$numentries&&$f<=$start+7*$numentries)
     {
        if($f%$numentries==0)
        {
           print "<A href='index.php?membername=$membername&start=$d'>$g</a> ";
        }
     }
     $d=$d+1;
     $g=1+$d/$numentries;
  ...
  {
    print "<A href='index.php?membername=$membername&start=$next'>>></a>&nbsp;";
  }
  print "</td>";
  print "<td width=30%><center><A href='../members.php?membername=$membername'>Back to main</a></center></td>";
  print "<td valign='top' width='20%'>";
  print "<center><A href='addentry.php?membername=$membername'>[Sign Guestbook]</center></a>";
  print "</td></tr></table>";
  print "</td>";
  $gbvar="Select * FROM bl_gbook where identifier='$getvars3[adminid]' order by ID DESC LIMIT " . $start . ", $numentries"; //retrieve entries from sql
  ...
  while($gbvalues=mysql_fetch_array($row))
  {
    ...
    if($getadmin2[adminid]==$gbvalues[identifier])
    {
      print "<br><br><A href='edit.php?membername=$blogadmin&ID=$gbvalues[ID]'>Edit</a>&nbsp;&nbsp;<A href='delete.php?membername=$blogadmin&ID=$gbvalues[ID]'>Delete</a>";
    }
    print "</td></tr>";
