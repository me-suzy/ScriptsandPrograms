<?php 
$getcalendar="SELECT calid,dateclass from bl_calender where personid='$getvars3[adminid]'";
$getcalendar2=mysql_query($getcalendar) or die(mysql_error());
...
$previous_link = "<a href=\"".$_SERVER['PHP_SELF']."?membername=$membername&date="; 
if($month == 1){ 
...
$next_link = "<a href=\"".$_SERVER['PHP_SELF']."?membername=$membername&date="; 
if($month == 12){ 
...
while (list(, $week) = each($weeks))
{
     ...
     { 
         ...
         { 
            ...
            $eventcount=mysql_num_rows($getcalendar2);
            $isevent=0;
            while($getcalendar3=mysql_fetch_array($getcalendar2))
            {
               if($getcalendar3[dateclass]==$datecount)
               {
                  $isevent=1;
               }
            ...
            if($eventcount>0)
            {
               mysql_data_seek($getcalendar2,0);
            }
            if($isevent==1)
            {
              $day_link = "<A href=\"javascript:popWin('viewcalender.php?ID=$datecount&membername=$membername',350, 400)\">$d</a>";
           ...
           { 
             if($isevent==1)
             {
                echo "<td><A href=\"javascript:popWin('viewcalender.php?ID=$datecount&membername=$membername',350, 400)\"><font color='red'><b>$d</b></font></a></td>\n"; 
             }
             else
             {
                 echo"<td><b>$d</b></td>\n"; 
