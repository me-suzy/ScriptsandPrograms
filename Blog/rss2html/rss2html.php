...
//
// $Id: rss2html.php,v 2.10 2005/10/22 18:51:47 housley Exp $
//
// $Log: rss2html.php,v $
...
if (function_exists("FeedForAll_rss2html_readFile") === FALSE) {
  Function FeedForAll_rss2html_readFile($filename, $useFopenURL) {
    $GLOBALS["ERRORSTRING"] = "";
    $result = "";
    if (stristr($filename, "://")) {
      if ($useFopenURL) {
...
if (!isset($_REQUEST["buildURL"])) {
  ...
  if (($XML = FeedForAll_rss2html_readFile($XMLfilename, $useFopenURL)) === FALSE) {
    if ($GLOBALS["ERRORSTRING"] == "") {
      echo "Unable to open RSS Feed $TEMPLATEfilename, exiting\n";
    } else {
      echo "Unable to open RSS Feed $TEMPLATEfilename with error <b>$GLOBALS[ERRORSTRING]</b>, exiting\n";
    }
    exit -1;
  }
  ...
  echo "<!-- HTML generated from an RSS Feed by rss2html.php, http://www.FeedForAll.com/ a NotePage, Inc. product (http://www.notepage.com/) -->";
  echo $template;
} else {
