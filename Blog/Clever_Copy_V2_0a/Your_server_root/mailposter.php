...
if (strlen($bannedip)>1)
    {
      ...
      {
         echo "<table border='0' width=100%><tr><td><center><b>$mail_poster_label</b><br>$abuse_prevention_label $ips<br>*$required_label<br><br><br></td></tr>";
         echo "<tr><td>";
         echo "<form method='post' action='mailposter.php' name='form'>";
         ...
         echo "<input type='hidden' name='ad'  value = $destination_address>";
      ...
      {
         echo "<center>";
         echo "<table border='0' width=95%><tr><td><center><b>$mail_poster_label</b></center></td></tr>";
         echo "<tr><td>";
         $name=antihax($_POST['name']);
         $email=antihax($_POST['email']);
         $www=antihax($_POST['www']);
         $comment=antihax($_POST['comment']);
         $destination_address = ($_POST['ad']);
         if((strlen($name)<1 || strlen($comment)<1 || strlen($email)<1))
         {
          echo "<b>$missing_post_data_error</b><br>";
         }else{
          $ip=$_SERVER["REMOTE_ADDR"];
     @mail("$destination_address","$mail_poster_subject_label $sitename","$mail_poster_comment_email\n\nFrom $name - $ip. Web - http://$www\n\n$comment","FROM: $email");
     echo "<p align = 'center'>$contact_sent_successfully_label <META HTTP-EQUIV = 'Refresh' Content = '2; URL =index.php'>";
