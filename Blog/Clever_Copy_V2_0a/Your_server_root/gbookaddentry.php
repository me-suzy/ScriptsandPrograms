...
if (strlen($bannedip)>1)
    {
      ...
      {
         echo "<table border='0' width=100%><tr><td><center><b>$guestbook_title_label</b><br>$guestbook_email_www_label<br><br></td></tr>";
         echo "<tr><td>";
         echo "<form method='post' action='gbookaddentry.php' name='form'>";
         ...
         if ($_SESSION['cuser'])
         {
            ...
            echo "<center>$vis_post_news_url_label<br><input type='text' name='www' size='60' value = '$wwaddy'><br>";
            echo "<center>$your_mail_addy_label<br><input type='text' name='email' size='60' value  ='$mailaddy'><br>";
         }else{
         ...
         echo "<center><textarea rows='6' name='comment' cols='60'></textarea><br>";
      ...
      {
         echo "<center>";
         echo "<table border='0' width=95%><tr><td><center><b>$guestbook_title_label</b></center></td></tr>";
         echo "<tr><td>";
      ...
      if(strlen($name)<1 || strlen($comment)<1)
      {
         echo "<b>$missing_post_data_error</b><br>";
          ...
          if ($getprefs3[dateset] =="1"){
               ...
               $thetime=date("D M jS Y - H:i",time() + $theoffset);
          }else{
               $theoffset = $getprefs3[time_offset];
               $theoffset = ($theoffset *'60');
               $thetime=date("D jS M Y - H:i",time() + $theoffset);
     }
     $addtogb="INSERT INTO CC_guestb(name,  mail, www, comment,  time,IP) VALUES('$name','$email','$www','$comment','$thetime','$ip')";
     mysql_query($addtogb);
     @mail("$siteemail","$gbook_added_email_subject $sitetitle","$gbook_comment_email\n\n$comment","FROM: $siteemail");
     echo "$comment_posted_successfully_label <META HTTP-EQUIV = 'Refresh' Content = '0; URL =gbindex.php'>";
     echo "<p align = 'center'>$if_you_see_label <A href='gbindex.php'> $click_here_label</a></p>";
    }
