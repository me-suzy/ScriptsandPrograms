...
{
   $ID = $_GET[ID];
   echo "<table border='0' width=100%><tr><td><center><b>$site_newswatch_label</b><br></td></tr>";
   echo "<tr><td>";
   echo "<form method='post' action='watchnews.php' name='form'>";
   echo "<center>* $your_mail_addy_label<br><input type='text' name='email' size='60'><br>";
   echo "<input type='hidden' name='masterid' value='$ID'>";
...
{
   echo "<center>";
   echo "<table border='0' width=95%><tr><td><center><b>$site_newswatch_label</b></center></td></tr>";
   echo "<tr><td>";
   $masterid = $_POST['masterid'];
   $email=antihax($_POST['email']);
   if(strlen($email)<1)
   {
      echo "<p align = 'center'><br><b>$missing_post_data_error </b><META HTTP-EQUIV = 'Refresh' Content = '3; URL =watchnews.php'>";
      echo "<p align = 'center'>$if_you_see_label <A href='index.php'> $click_here_label</a></p>";
      exit;
   }
   $testquery =  ("SELECT * FROM CC_watchnews") or die($no_watchers_error);
   $testresult = mysql_query($testquery);
   $exists = "false";
   while($testrow = mysql_fetch_array($testresult)){
          if (($email == $testrow[address])&& ($masterid == $testrow[masterid]))
          {
                $exists = "true";
          }
   }
   if ($exists == "true")
   {
      echo "<p align = 'center'><br><b>$site_watch_already_subscribed_label </b><META HTTP-EQUIV = 'Refresh' Content = '3; URL =index.php'>";
      echo "<p align = 'center'>$if_you_see_label <A href='index.php'> $click_here_label</a></p>";
      exit;
   }else{
      @mysql_query("INSERT INTO CC_watchnews (address, masterid) VALUES('$email','$masterid')" );
      $error_msg .= mysql_error();
   }
   if ($error_msg == "")
   {
         echo "<p align = 'center'><br><b>$site_newswatch_successful_label </b><META HTTP-EQUIV = 'Refresh' Content = '3; URL =index.php'>";
         echo "<p align = 'center'>$if_you_see_label <A href='index.php'> $click_here_label</a></p></table>";
   }else{
         echo "<p align = 'center'><br><b>$site_watch_error_label $error_msg</b><META HTTP-EQUIV = 'Refresh' Content = '5; URL =watchnews.php'>";
         echo "<p align = 'center'>$if_you_see_label <A href='index.php'> $click_here_label</a></p></table>";
   }
}
echo "</center></td>";
