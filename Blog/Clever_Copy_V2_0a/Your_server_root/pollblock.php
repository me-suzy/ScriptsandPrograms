<?php
$getpoll="SELECT * FROM CC_poll";
$getpoll2=mysql_query($getpoll) or die($no_poll_error);
$getpoll3=mysql_fetch_array($getpoll2);
$pollctr = '0';
for($i = 1; $i <= 8; $i++)
{
   $thisanswer = $getpoll3[answer.$i];
   if ($thisanswer !== "")
   {
         $pollctr++;
   }
...
if ($getprefs3[useblockwrapper]==1)
{
   ...
   echo "<font face=$getprefs3[block_heading_font_face] size=$getprefs3[block_heading_font_size] color=$getprefs3[block_heading_font_color]><$getprefs3[block_heading_font_decoration]>&nbsp;$getblocks3[poll_block_label]</font></b></center></td></tr>";
   echo "<tr><td bgcolor=$getprefs3[block_background_color]>";
   echo "<center>The poll block is not available on this site";
   echo "</td></tr></table><br>";
}else{
   echo "<center><font face = $getprefs3[block_heading_font_face] size = $getprefs3[block_heading_font_size] color= $getprefs3[block_heading_font_color]><$getprefs3[block_heading_font_decoration]>$getblocks3[poll_block_label]</$getprefs3[block_heading_font_decoration]></center>"; 
