...
if(isset($_SESSION['cuser']))
 ...
 echo "<tr><td colspan='3'><center><b>$edit_diary_heading_label</b><br><br></center></td></tr>";
 echo "<tr><td colspan = '3' valign='top' align = 'left'>";
 if($_SESSION['cuser'])
 {
  $ID=$_GET['ID'];
  $journal="SELECT * FROM CC_journal WHERE user = '$cuser' AND id = '$ID'";
  $row=mysql_query($journal);
  echo "<form action='editjournal.php?op=edit' method='post' name='form'>";
  while($journalvalues=mysql_fetch_array($row))
  {
   $title = $journalvalues['title'];
   echo "<center>$edit_this_diary_title_label<br>";
   echo "<textarea name='title' rows='1' cols='100'> $journalvalues[title]</textarea><br><br>";
   echo "$edit_diary_entry_label<br>";
   echo "<textarea name='entry' rows='5' cols='100'> $journalvalues[entry]</textarea><br><br>";
  ...
  echo "<center><input type='submit' name='submit' value='$confirm_diary_button_label'class = 'buttons'></form>";
 }else{
 ...
 echo "<center><form><input type='button' class = 'buttons' value='$back_label' onclick='history.back()'></form>";
 echo "</center>";
 switch( $_GET[ "op" ] )
 {
 case "edit":
 $ID = antihaxmjr($_POST['id']);
 $entry = antihaxmjr($_POST['entry']);
 $title = antihaxmjr($_POST['title']);
 $public = antihaxmjr($_POST['public']);
 @mysql_query ("UPDATE CC_journal SET title='$title',entry='$entry', public='$public' WHERE id ='$ID' AND user = '$visitor_name'");
 $error_msg .= mysql_error();
 if ($error_msg == "")
 {
      echo "<center><b><br>$diary_edit_ok_label<br></b>";
      ...
      echo "<center>$there_was_problem_label - $error_msg";
      echo "<center>$if_you_see_label <a href='myjournal.php'>$click_here_label</a>";
 }
 break;
 }
