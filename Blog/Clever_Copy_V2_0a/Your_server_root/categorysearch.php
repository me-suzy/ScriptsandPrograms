...
$query = "SELECT * FROM CC_news WHERE  category = '$category' AND (introcontent LIKE '%".$searchterm."%' || newstitle LIKE '%".$searchterm."%' || maincontent LIKE '%".$searchterm."%' || author LIKE '%".$searchterm."%') ORDER BY realtime DESC";
...
$query = "SELECT * FROM CC_news WHERE  category = '$category' AND (introcontent LIKE '%".$searchterm."%' || newstitle LIKE '%".$searchterm."%' || maincontent LIKE '%".$searchterm."%' || author LIKE '%".$searchterm."%') ORDER BY realtime DESC LIMIT $start,$showmax";
$result = mysql_query($query);
$num_results = mysql_num_rows($result);
...
$order="SELECT * FROM CC_news WHERE  category = '$category' AND (introcontent LIKE '%".$searchterm."%' || newstitle LIKE '%".$searchterm."%' || maincontent LIKE '%".$searchterm."%' || author LIKE '%".$searchterm."%') ORDER BY realtime DESC";
...
if($start>=$showmax)
{
    echo "<A href='categorysearch.php?start=0&searchterm=$searchterm&category=$category'><-</a>&nbsp;";
}
while($order3=mysql_fetch_array($order2))
{
   if($fixed>=$start-3*$showmax&&$fixed<=$start+7*$showmax)
   {
      if($fixed%$showmax==0)
      {
         echo "<A href='categorysearch.php?start=$ctr&searchterm=$searchterm&category=$category'>$md</a>&nbsp;";
         $totalpages = $md;
      }
   }
   $ctr=$ctr+1;
...
{
   $end=$fixed-$showmax;
   echo "<A href='categorysearch.php?start=$end&searchterm=$searchterm&category=$category'>-></a>&nbsp;";
}
