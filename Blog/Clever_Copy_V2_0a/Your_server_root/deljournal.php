...
if(isset($_SESSION['cuser']))
 ...
 echo "<tr><td colspan='3'><center><b>$del_diary_heading_label</b><br><br></center></td></tr>";
 echo "<tr><td colspan = '3' valign='top' align = 'left'>";
 if($_SESSION['cuser'])
 {
  $ID=$_GET['ID'];
  $tjournal="SELECT * FROM CC_journal WHERE user = '$cuser' AND id = '$ID'";
  $trow=mysql_query($tjournal);
  echo "<form action='deljournal.php?op=del' method='post' name='form'>";
  while($tjournalvalues=mysql_fetch_array($trow))
  {
     echo "$tjournalvalues[title]";
  }
  echo "<input type='hidden' name='id' value = '$ID'>";
  echo "<center><input type='submit' name='submit' value='$del_diary_button_label'class = 'buttons'></form>";
 }else{
 ...
 @mysql_query( "DELETE FROM CC_journal WHERE id = '$ID'");
 $error_msg .= mysql_error();
 if ($error_msg == "")
 {
      echo "<center><b><br>$diary_del_ok_label<br></b>";
