...
if($getadmin3['status']==3)
{
   ...
   $getblock="SELECT * from CC_custom_centre_blocks WHERE customid = '$editthisblock'";
   $getblock2=mysql_query($getblock) or die($no_customblock_error);
   $getblock3=mysql_fetch_array($getblock2);
   echo "<center><form action='editcustomcentreblock.php?op=edit_block2nd' method='post'>";
   ...
   $ID = $_POST[ID];
   $ublock="UPDATE `CC_custom_centre_blocks` SET `customblock` = '$blockcontent' WHERE`customid`  = '$ID'";
