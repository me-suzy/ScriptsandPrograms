...
<?php
$date = mktime();
$query="SELECT ID, last_update, owner, paused,never_expire, date_limit_on, start_date, end_date, clickthru_limit_on, clickthru_limit, clickthru, view_limit, views, view_limit_on, bannerfile, bannerurl, banneralt, active, refresh FROM CC_banners";
...
if (($row[active]=='1') && ($row[view_limit_on]=='1') && ($row[views]<= $row[view_limit]) && ($row[paused]!== '1')){
        ?>
        <br><a title= '<?=$row['banneralt']?>' href="banner_opener.php?ID=<?php echo $row['ID']; ?>" onClick="JavaScript:refresh()" target="_blank"><img src="<?=$sitebanner?>" border=0/></a>
        <?php
        mysql_query("UPDATE CC_banners SET views = views + 1 WHERE ID = ". $row['ID']);
        if (($row[views]>= $row[view_limit])){
              mysql_query("UPDATE CC_banners SET active = '0', end_date = '$date' WHERE ID = ". $row['ID']);
        }
        $thisviews = $row[views];
        $thisowner = $row[owner];
   ...
   echo "<br>$error_msg<br>";
    ...
    $query="SELECT bannerfile, bannerurl, banneralt, active, refresh FROM CC_banners WHERE ID ='1'";
    $res = @mysql_query($query);
    $row = mysql_fetch_array($res);
    ?>
    <br><a title= '<?=$row['banneralt']?>' href="banner_opener.php?ID=1" onClick="JavaScript:refresh()" target="_blank"><img src="<?=$sitebanner?>" border=0/></a>
    <?php
    if  ($diff > 86400)
    {
      if ($last_update == '0')
      {
        mysql_query("UPDATE CC_banners SET last_update = '$now_time' WHERE ID = '$ID'");
        $error_msg .= mysql_error();
      }
      else
      {
      $end_date=$enddate+ 86400;
      mysql_query("UPDATE CC_banners SET last_update = '$now_time', end_date = '$end_date' WHERE ID = '$ID'");
      $error_msg .= mysql_error();
      echo "<br>$error_msg<br>";
      }
    }
