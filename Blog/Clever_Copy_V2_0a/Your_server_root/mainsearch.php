...
$query = "SELECT * FROM CC_news WHERE  introcontent LIKE '%".$searchterm."%' || newstitle LIKE '%".$searchterm."%' || maincontent LIKE '%".$searchterm."%' || author LIKE '%".$searchterm."%'  || authweb LIKE '%".$searchterm."%'  || thetime LIKE '%".$searchterm."%'  || year LIKE '%".$searchterm."%' || month LIKE '%".$searchterm."%'ORDER BY realtime DESC";
$result = mysql_query($query);
$num_results = mysql_num_rows($result);
echo "<table width = '100%' border = '0'><tr><td>";
echo "<br><b><center>$searches_found_label - $num_results</center></b><br><br>";
echo "<b><center>$search_results_label $searchterm </center></b>";
echo "<b><center>$most_recent_results_label</center></b><br><hr></td></tr><tr><td>";
$query = "SELECT * FROM CC_news WHERE  introcontent LIKE '%".$searchterm."%' || newstitle LIKE '%".$searchterm."%' || maincontent LIKE '%".$searchterm."%' || author LIKE '%".$searchterm."%'  || authweb LIKE '%".$searchterm."%'  || thetime LIKE '%".$searchterm."%'  || year LIKE '%".$searchterm."%' || month LIKE '%".$searchterm."%'ORDER BY realtime DESC LIMIT $start,$showmax ";
$result = mysql_query($query);
$num_results = mysql_num_rows($result);
...
$order="SELECT * FROM CC_news WHERE  introcontent LIKE '%".$searchterm."%' || newstitle LIKE '%".$searchterm."%' || maincontent LIKE '%".$searchterm."%' || author LIKE '%".$searchterm."%'  || authweb LIKE '%".$searchterm."%'  || thetime LIKE '%".$searchterm."%'  || year LIKE '%".$searchterm."%' || month LIKE '%".$searchterm."%'ORDER BY realtime DESC";
...
if($start>=$showmax)
{
    echo "<A href='mainsearch.php?start=0&searchterm=$searchterm'><-</a>&nbsp;";
}
while($order3=mysql_fetch_array($order2))
{
   if($fixed>=$start-3*$showmax&&$fixed<=$start+7*$showmax)
   {
      if($fixed%$showmax==0)
      {
         echo "<A href='mainsearch.php?start=$ctr&searchterm=$searchterm'>$md</a>&nbsp;";
         $totalpages = $md;
      }
   }
   $ctr=$ctr+1;
...
{
   $end=$fixed-$showmax;
   echo "<A href='mainsearch.php?start=$end&searchterm=$searchterm'>-></a>&nbsp;";
}
