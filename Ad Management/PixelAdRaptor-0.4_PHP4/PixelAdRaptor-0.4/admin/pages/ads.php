...
/**
 *
 * Administration's ads section handler
 *
 * @date 2005-10-15
 * @file ads.php
 *
 ...
 * Get approved ads
 */
$q = 'SELECT id, name, email, FLOOR(x / 10) + 1 as x, FLOOR(y / 10) + 1 as y, title, link, size, file, date, hits, price FROM ads WHERE active = 1 ORDER BY size DESC';
...
if (isset($_GET['act']) && $_GET['act'] == 'disable' && isset($_GET['id']) && is_numeric($_GET['id'])) {

    $q = 'UPDATE ads SET active = 0 WHERE id = "'.$_GET['id'].'"';
    $api['database']->query($q);
