...
function phpAds_buildQuery ($part, $lastpart, $precondition)
{
	...
	if ($part != '')
	{
		...
		for ($k=0; $k < count($part_array); $k++)
		{
			...
			if($part_array[$k] != '' && $part_array[$k] != ' ')
			{
				...
				else
				{
					...
					{
						if ($operator == 'OR')
							$conditions .= "OR LOWER(' ' || ".$phpAds_config['tbl_banners'].".keyword || ' ') LIKE LOWER('% $part_array[$k] %') ";
						elseif ($operator == 'AND')
							$conditions .= "AND LOWER(' ' || ".$phpAds_config['tbl_banners'].".keyword || ' ') LIKE LOWER('% $part_array[$k] %') ";
						else
							$conditions .= "AND LOWER(' ' || ".$phpAds_config['tbl_banners'].".keyword || ' ') NOT LIKE LOWER('% $part_array[$k] %') ";
					}
				}
			}
		}
		...
		if ($lastpart == true && $onlykeywords == true)
			$conditions .= "OR LOWER(' ' || ".$phpAds_config['tbl_banners'].".keyword || ' ') LIKE '% global %' ";
