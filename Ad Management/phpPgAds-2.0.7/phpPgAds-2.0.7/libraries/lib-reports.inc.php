...
function phpAds_SendMaintenanceReport ($clientid, $first_unixtimestamp, $last_unixtimestamp, $update=true)
{
	...
	if (phpAds_dbNumRows($res_client) > 0)
	{
		...
		$date_format = array(strtr($date_format, array("%d" => "DD", "%m" => "MM", "%Y" => "YYYY")), $date_format);
				...
				if ($adviews > 0)
				{
					...
					if ($phpAds_config['compact_stats'])
						$res_adviews = phpAds_dbQuery("
							SELECT
								SUM(views) as qnt,
								DATE_FORMAT(day, '$date_format[0]') as t_stamp_f,
						...
						$res_adviews = phpAds_dbQuery("
							SELECT
								count(*) as qnt,
								DATE_FORMAT(t_stamp::date, '$date_format[0]') as t_stamp_f,
								t_stamp::date AS the_day
							FROM
		...
		if ($client["email"] != '' && $active_campaigns == true)
		{
			...
			$Body	 = str_replace ("{startdate}", 		date(str_replace('%', '', $date_format[1]), $first_unixtimestamp), $Body);
			$Body	 = str_replace ("{enddate}", 		date(str_replace('%', '', $date_format[1]), $last_unixtimestamp), $Body);
