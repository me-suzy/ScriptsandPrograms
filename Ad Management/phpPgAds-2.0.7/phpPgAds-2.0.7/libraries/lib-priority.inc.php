...
function phpAds_PriorityGetImpressions($days, $offset)
{
	...
	{
		$begin = date('Y-m-d', phpAds_makeTimestamp(mktime (0, 0, 0, date('m'), date('d'), date('Y')), - 1 - $offset));
		$end   = date('Y-m-d', phpAds_makeTimestamp(mktime (0, 0, 0, date('m'), date('d'), date('Y')), - (60 * 60 * 24 * $days) - $offset));
	...
	{
		$begin = date('Y-m-d H:i:s', phpAds_makeTimestamp(mktime (0, 0, 0, date('m'), date('d'), date('Y')), - 1 - $offset));
		$end   = date('Y-m-d H:i:s', phpAds_makeTimestamp(mktime (0, 0, 0, date('m'), date('d'), date('Y')), - (60 * 60 * 24 * $days) - $offset));
...
function phpAds_PriorityPredictProfile($campaigns, $banners)
{
	...
	if ($phpAds_config['compact_stats'])
	{
		$res = phpAds_dbQuery("SELECT UNIX_TIMESTAMP(day) AS days_running FROM ".$phpAds_config['tbl_adstats']." ORDER BY day LIMIT 1");
		$days_running = phpAds_dbResult($res, 0, 'days_running');
	}
	else
	{
		$res = phpAds_dbQuery("SELECT UNIX_TIMESTAMP(t_stamp) AS days_running FROM ".$phpAds_config['tbl_adviews']." ORDER BY t_stamp LIMIT 1");
		$days_running = phpAds_dbResult($res, 0, 'days_running');
	}
	...
	if ($phpAds_config['compact_stats'])
	{
		$begin = date('Y-m-d', mktime (0, 0, 0, date('m'), date('d'), date('Y')));
	...
	else
	{
		...
		$end   = date('Y-m-d H:i:s', mktime (phpAds_CurrentHour, 0, 0, date('m'), date('d'), date('Y')) - 1);
...
function phpAds_PriorityPrepareBanners()
{
	...
	{
		$begin = date ('Y-m-d', mktime (0, 0, 0, date('m'), date('d'), date('Y')));
		$end   = date ('Y-m-d', mktime (phpAds_CurrentHour, 0, 0, date('m'), date('d'), date('Y')) - 1);
		...
		foreach (array_keys($campaigns) as $c)
		{
			if ($campaigns[$c]['target'] > 0)
			{
				...
				if ($period > 0)
				{
					...
					{
						$compensation_factor = 'not available, average impressions per hour is null';
						$extra_to_assign = 0;
					}
