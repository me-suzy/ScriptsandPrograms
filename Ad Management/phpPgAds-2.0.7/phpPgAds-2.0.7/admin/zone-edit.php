...
if (isset($submit))
{
	...
	else
	{
		...
		$zoneid = phpAds_dbInsertID($phpAds_config['tbl_zones'].'_zoneid_seq');
...
$res = phpAds_dbQuery("SELECT * FROM ".$phpAds_config['tbl_zones']." WHERE affiliateid = '".$affiliateid."'".(isset($zoneid) && $zoneid ? " AND zoneid != '".$zoneid."'" : ''));
while ($row = phpAds_dbFetchArray($res))
