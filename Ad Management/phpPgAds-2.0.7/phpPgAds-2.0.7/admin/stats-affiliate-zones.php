...
if (!$phpAds_config['compact_stats'])
{
	switch ($period)
	{
		case 'r':	$limit 		    	= " AND t_stamp >= ".date("'Y-m-d H:i:s'", $period_begin)." AND t_stamp < ".date("'Y-m-d H:i:s'", $period_end);
					break;
				
		case 'y':	$timestamp_begin	= mktime(0, 0, 0, date('m'), date('d') - 1, date('Y'));	
					$timestamp_end		= mktime(0, 0, 0, date('m'), date('d'), date('Y'));
					$limit 		    	= " AND t_stamp >= ".date("'Y-m-d H:i:s'", $timestamp_begin)." AND t_stamp < ".date("'Y-m-d H:i:s'", $timestamp_end);
					break;
				
		case 't':	$timestamp_begin	= mktime(0, 0, 0, date('m'), date('d'), date('Y'));
					$limit 				= " AND t_stamp >= ".date("'Y-m-d H:i:s'", $timestamp_begin);
					break;
...
else
{
	switch ($period)
	{
		case 'r':	$limit 		    	= " AND day >= ".date("'Y-m-d'", $period_begin)." AND day < ".date("'Y-m-d'", $period_end);
					break;
				
		case 'y':	$timestamp_begin	= mktime(0, 0, 0, date('m'), date('d') - 1, date('Y'));
					$timestamp_end		= mktime(0, 0, 0, date('m'), date('d'), date('Y'));
					$limit 				= " AND day >= ".date("'Y-m-d'", $timestamp_begin)." AND day < ".date("'Y-m-d'", $timestamp_end);
					break;
				
		case 't':	$timestamp_begin	= mktime(0, 0, 0, date('m'), date('d'), date('Y'));
					$limit 				= " AND day >= ".date("'Y-m-d'", $timestamp_begin);
					break;
