...
function phpAds_ImageStore ($storagetype, $name, $buffer, $overwrite = false)
{
	...
	if ($storagetype == 'sql')
	{
		if ($overwrite == false)
			...
			$old_oid = phpAds_dbResult(phpAds_dbQuery("
				SELECT
					contents
				FROM
		...
		if ($oid = phpPgAds_lo_create($buffer))
			$buffer = $oid;
		...
		if (!isset($old_oid))
		{
			$res = phpAds_dbQuery ("
				INSERT INTO 
					".$phpAds_config['tbl_images']." (filename, contents, t_stamp)
				VALUES ('".$name."', ".$buffer.", now())
			");
		}
		else
		{
			if ($old_oid)
			{
				...
				@pg_lounlink($GLOBALS['phpAds_db_link'], $old_oid);
...
function phpAds_ImageRetrieve ($storagetype, $name)
{
	...
	if ($storagetype == 'sql')
	{
		...
		{
			$result = phpPgAds_lo_read($row['contents']);
		}
	}
...
function phpAds_ImageDelete ($storagetype, $name)
{
	...
	if ($storagetype == 'sql')
	{
		$oid = phpAds_dbResult(phpAds_dbQuery ("
		...
		{
			phpAds_dbQuery("BEGIN");
			@pg_lounlink($GLOBALS['phpAds_db_link'], $oid);
			phpAds_dbQuery("COMMIT");
		}
	}
}
...
function phpAds_ImageSize ($storagetype, $name)
{
	...
	if ($storagetype == 'sql')
	{
		...
		if ($row = phpAds_dbFetchArray($res))
		{
			$result = strlen(phpPgAds_lo_read($row['contents']));
		}
	}
