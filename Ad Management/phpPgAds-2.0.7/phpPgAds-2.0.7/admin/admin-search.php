...
<html<?php echo $phpAds_TextDirection != 'ltr' ? " dir='".$phpAds_TextDirection."'" : '' ?>>
	<head>
		...
		<meta name='author' content='phpPgAds - http://sourceforge.net/projects/phppgads'>
...
<?php	
 	if ($phpAds_config['name'] != "")
	{
		...
		echo "<span class='phpPgAds'>&nbsp;&nbsp;&nbsp;$phpAds_name &nbsp;&nbsp;&nbsp;</span>";
	}
	else
	{
...
<?php
	$upper_keyword = strtoupper($keyword);

	$query_clients = "SELECT * FROM ".$phpAds_config['tbl_clients']." WHERE UPPER(clientname) LIKE '%" . $upper_keyword . "%' AND parent IS NULL";
  	$res_clients = phpAds_dbQuery($query_clients) or phpAds_sqlDie();
	
	$query_campaigns = "SELECT * FROM ".$phpAds_config['tbl_clients']." WHERE UPPER(clientname) LIKE '%" . $upper_keyword . "%' AND parent > 0";
  	$res_campaigns = phpAds_dbQuery($query_campaigns) or phpAds_sqlDie();
	
	$query_banners = "SELECT * FROM ".$phpAds_config['tbl_banners']." WHERE UPPER(alt) LIKE '%" . $upper_keyword . "%' OR UPPER(description) LIKE '%" . $upper_keyword . "%'";
  	$res_banners = phpAds_dbQuery($query_banners) or phpAds_sqlDie();
	
	$query_affiliates = "SELECT * FROM ".$phpAds_config['tbl_affiliates']." WHERE UPPER(name) LIKE '%".$upper_keyword."%'";
  	$res_affiliates = phpAds_dbQuery($query_affiliates) or phpAds_sqlDie();
	
	$query_zones = "SELECT * FROM ".$phpAds_config['tbl_zones']." WHERE UPPER(zonename) LIKE '%".$upper_keyword."%' OR UPPER(description) LIKE '%".$upper_keyword."%'";
