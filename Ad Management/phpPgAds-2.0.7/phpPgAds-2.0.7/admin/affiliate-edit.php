...
if (isset($submit))
{
	...
	if (count($errormessage) == 0)
	{
		...
		if (!isset($affiliateid) || $affiliateid == '')
		{
			...
			$affiliateid = phpAds_dbInsertID($phpAds_config['tbl_affiliates'].'_affiliateid_seq');
...
$res = phpAds_dbQuery("SELECT * FROM ".$phpAds_config['tbl_affiliates'].(isset($affiliateid) && $affiliateid ? " WHERE affiliateid != '".$affiliateid."'" : ''));
while ($row = phpAds_dbFetchArray($res))
...
$res = phpAds_dbQuery("SELECT * FROM ".$phpAds_config['tbl_affiliates']." WHERE username != ''".(isset($affiliateid) && $affiliateid ? " AND affiliateid != '".$affiliateid."'" : ''));
while ($row = phpAds_dbFetchArray($res))
	$unique_users[] = $row['username'];

$res = phpAds_dbQuery("SELECT * FROM ".$phpAds_config['tbl_clients']." WHERE clientusername != '' AND parent IS NULL");
while ($row = phpAds_dbFetchArray($res))
