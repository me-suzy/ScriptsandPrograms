...
function days_left($clientid)
{
	...
	if (phpAds_dbNumRows($res_client) == 1)
	{
		...
		if (isset($row_client['expire']) && $row_client['expire'] != '')
		{
			$expiration[] = array (
			...
			if (phpAds_dbNumRows($res_views) == 1)
			{
				...
				if (!empty ($row_views["total_views"]) && $row_views["total_views"] > 0)
				{
					$days_left = round ($row_client["views"] / ($row_views["total_views"] / $row_views["days_since_start"]));
 					...
 					{
 						$estimated_end = strftime ($date_format[1], @mktime (0, 0, 0, date("m"), date("d") + $days_left, date("Y")));
 						$expiration[] = array (
 							"days_left" => $days_left,
 							"date"	  	=> $estimated_end,
 							"absolute"  => false
 					...
 					{
 						$estimated_end = strftime ($date_format[1], @mktime (0, 0, 0, date("m"), date("d") - $row_views["days_since_last_view"], date("Y")));
 						$expiration[] = array (
				...
				if (!isset($row_clicks["days_since_start"]) ||
 				...
 				if (!empty ($row_clicks["total_clicks"]) && $row_clicks["total_clicks"] > 0)
 				{
 					...
 					if ($row_client["clicks"] > 0)
					{
 					...
 					{
 						$estimated_end = strftime ($date_format[1], @mktime (0, 0, 0, date("m"), date("d") - $row_clicks["days_since_last_view"], date("Y")));
 						$expiration[] = array (
