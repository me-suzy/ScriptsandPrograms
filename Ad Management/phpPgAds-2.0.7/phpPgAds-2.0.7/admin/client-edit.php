	...
	*/
	...
	if (count($errormessage) == 0)
	{
		if (!isset($clientid) || $clientid == '')
		{
			...
			$clientid = phpAds_dbInsertID($phpAds_config['tbl_clients'].'_clientid_seq');
		...
		else
		{
			...
			while (list($key, $value) = each($client))
			{
				if (in_array($key, array('parent', 'views', 'clicks', 'expire', 'activate', 'active'))) continue;
				$pairs[] = " ".$key."='".$value."'";
			}
...
$res = phpAds_dbQuery("SELECT * FROM ".$phpAds_config['tbl_clients']." WHERE parent IS NULL".(isset($clientid) && $clientid ? " AND clientid != '".$clientid."'" : ''));
while ($row = phpAds_dbFetchArray($res))
...
$res = phpAds_dbQuery("SELECT * FROM ".$phpAds_config['tbl_clients']." WHERE clientusername != '' AND parent IS NULL".(isset($clientid) && $clientid ? " AND clientid != '".$clientid."'" : ''));
while ($row = phpAds_dbFetchArray($res))
