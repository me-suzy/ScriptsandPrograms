...
// Pre configmanager check
$pre_configmanager = defined('phpAds_installed');
...
else
{
	...
	$res = $pre_configmanager ||
		phpAds_dbQuery("SELECT COUNT(*) FROM ".$phpAds_config['tbl_session']);
	
	if (!$res && phpAds_dbQuery("SELECT COUNT(*) FROM ".$phpAds_config['tbl_clients']))
	{
		// Create table
		$res = phpAds_dbQuery("
			CREATE TABLE ".$phpAds_config['tbl_session']."
...
if (phpAds_isUser(phpAds_Admin))
{
	...
	elseif ($upgrade && $step == 2)
	{
		...
		if (!$result)
			$continue = false;
		elseif (phpAds_databaseCheckSupported)
