...
if (isset($row['span']) && $row['span'] > 0)
{
	...
	$result = phpAds_dbQuery("
		SELECT
			...
			DATE_FORMAT(day, '".$formatted[0]."') AS date,
	...
	if (count($clientids))
	{
		...
		else
		{
			...
			$result = phpAds_dbQuery("
				SELECT
					COUNT(*) AS sum_views,
					DATE_FORMAT(t_stamp::date, '".$formatted[0]."') AS date,
					DATE_FORMAT(t_stamp::date, '".$unformatted."') AS date_u
				FROM
	...
	if ($period == 'w')
	{
		...
		for ($d=0;$d<$limit;$d++)
		{
			...
			echo $avgtarget."&nbsp;</td>";
			echo "<td align='".$phpAds_TextAlignRight."' nowrap  height='15' bgcolor='$bgcolortot'>".phpAds_formatNumber($totalweektarget)."&nbsp;</td>";
			echo "</tr>";
