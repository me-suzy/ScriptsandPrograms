...
$get_refs = @mysql_query("SELECT * FROM `referrals`");

if(@mysql_num_rows($get_refs) > 0){
	$showviews = 'Please Select A Referral To Remove:';
} else {
	$showviews = 'There are currently No Referrals To Remove!';
}	
?>
      ...
      <table width="100%" border="0" cellspacing="5" cellpadding="5">
        <tr>
          <td> <table width="100%" border="1" cellpadding="0" cellspacing="3" bordercolor="#0000FF" >
            ...
            <?php 		if(@mysql_num_rows($get_refs) > 0){?>
			<form name="viewreferral" method="post" action="<?php echo $_SERVER['PHP_SELF'];?>" >
              <table width="100%" border="1" align="center" cellpadding="5" cellspacing="0" bordercolor="#00CCFF">
                ...
                <?php  $garnum =0; //Make sure at least one item is selected
					$issel = NULL;
					while($each_ref = @mysql_fetch_array($get_refs)){ 
						if($garnum == 0){ $issel = 'checked'; } else { $issel =NULL;}
							$garnum = $garnum+1; ?>
                <tr> 
                  <td><div align="left"><strong> 
                      <input name="removeme" type="radio" title="Select This Referral To Remoev" value="<?php echo $each_ref[0];?>" <?php echo $issel;?>>
                      <?php					   
						   echo $each_ref[1];
					  ?>
                      </strong></div></td>
                ...
                <tr> 
                  <td> <div align="center"> 
                      ...
                      <input name="submitid" type="hidden" value="16">
                      <input type="submit" name="Submit" value="Remove Selected Referral"  title="Remove Selected Counter" >
                  </div></td>
                </tr>
              </table>
            </form>
