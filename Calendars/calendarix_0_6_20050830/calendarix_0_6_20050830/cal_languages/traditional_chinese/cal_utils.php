...
global $EVENTS_TB,$USER_TB,$CAT_TB,$mth,$week,$mweek,$todayclr,$viewcalok,$userid,$userview,$userlogin,$uname,$ugroup,$weekstartday,$overlibbgclr,$catview,$notimeentry ;
  ...
  for ($i=intval($weekstartday)+1;$i<=(intval($weekstartday) + 7);$i++){
    ...
    if (dayinweek($i) == 1)
      echo "<div class=dayfont>".$mweek[dayinweek($i)]."</div></td>"; // sunday
    else    
      echo "<div class=dayfont>".$mweek[dayinweek($i)]."</div></td>"; // rest of week
    }
  echo "</tr>\n\n<tr>";
...
global $viewtodaydate,$week,$mth,$d,$m,$y,$caleventapprove,$viewcalok,$viewweekok,$viewdayok,$viewcatsok,$addeventok,$addeventwin_w,$addeventwin_h,$administrationok,$catview,$uname,$publicview;
...
// display icons
echo " &nbsp; <a href='#' onclick=\"Javascript:window.open('yearcal.php?op=yearcal&ycyear=$y','popupyear','toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=600,height=600');\"><img src=\"images/year.gif\" border=0 alt='".translate("Current Year")."' width=16 height=16/></a>";
...
if ((($publicview==1)&&($uname!=""))||($publicview==0)) {
  if ($addeventok==1){
    echo " &nbsp; <a href='#' onclick=\"Javascript:window.open('cal_addevent.php?op=eventform&catview=$catview','popupwin','toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=".$addeventwin_w.",height=".$addeventwin_h."');\"><img src=\"images/add.gif\" border=0 alt='".translate("Add Event")."' width=16 height=16/></a>";
  }
}
