...
<h1>New Event&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<INPUT type="button" value="Save" id="saveevent" name="saveevent" language="javascript" onclick="cevent_onsubmit()">&nbsp;&nbsp;&nbsp;<INPUT type="button" value="Go to Calendar" id="doneevent" name="doneevent" LANGUAGE=javascript onclick="return doneevent_onclick()">
</h1>
<form method="<?php print $GLOBALS["postorget"]; ?>" name="cevent" id="cevent" action="<?php print $GLOBALS["idxfile"]; ?>">
...
<input type="hidden" name="aminnewevent" id="aminnewevent" value="1" />
<?php
...
<table border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse" width="100%">
  <tr>
    ...
    <td width="75%"><input title="Enter the Title of the event here. It will appear on the different Calendar views." name="nef[eventtitle]" size="32" id="eventtitle" maxLength="50"></td>
  </tr>
  <tr>
    <td width="25%"><b><?php print $curcalcfg["gcscoif_subtitletxt"]; ?></b></td>
    <td width="75%"><input title="Enter the <?php print $curcalcfg["gcscoif_subtitletxt"]; ?> of the event here. It will appear on the different Calendar views." name="nef[eventsubtitle]" size="32" id="eventsubtitle" maxLength="50"></td>
  </tr>
</table>
</div>
    </td>
...
<?php
    for($lc=0;$lc<47;$lc++) {
		print "        <option ";
	if(isset($functime)) {
            if(substr($functime,0,2)==substr($timear[$lc],0,2)) {
                print "selected ";
            }
	} else {
...
#    $sqlstr = "select * from ".$GLOBALS["tabpre"]."_user_cat where (uid = '".$cuser->gsv("cuid")."' and (calid = '".$cuser->gsv("curcalid")."' or calid = '0')) or calid='-2'";
    ...
    ?>
        ...
        <input type="hidden" name="nef[cat]" id="rcat" value="0">
</div>
    <table border="1" width="272px"><tr>
    <TD id="cexampcell">
    <font id="cexampfont" color="<?php print $curcalcfg["gcscocf_btxtcolor"]; ?>">This box will show you an example<br>of the category you choose.</font>
    </TD></tr></table>
    </td>
</tr>
        <tr>
...
<!-- onchange="return remindercontacts_onchange()" -->
</div>
          </td>
        </tr>
        <tr>
...
?>
        <hr size="1" />
        <INPUT title="Check this box to allow Event Reminder Subscription Options." type="checkbox" id="remsuballow" name="nef[remsuballow]" LANGUAGE=javascript onclick="return remsuballow_onclick()">
...
<div style="display: none" id="divremsubopts" title="Set Event Reminder Subscription Options.">
        <table border="0" cellspacing="1" cellpadding="0" width="100%">
        <tr>
...
#    $sqlstr = "select * from ".$GLOBALS["tabpre"]."_cal_event_rems where uid=0 and contyp='U' and approved=1 and pending=0 and confirmed=1 and calid='".$cuser->gsv("curcalid")."'";
/* example option

        <OPTION value="U1|Philip Boone|philip@boone.de|8|1">Philip Boone, philip@boone.de, 8 Days</OPTION>
        <OPTION value="U2|Philip Boone|philip@boone.de|8|1">Philip Boone, philip@boone.de, 8 Days</OPTION>
        <OPTION value="U3|Philip Boone|philip@boone.de|8|1">Philip Boone, philip@boone.de, 8 Days</OPTION>
        <OPTION value="U4|Philip Boone|philip@boone.de|8|1">Philip Boone, philip@boone.de, 8 Days</OPTION>
*/
?>
        </SELECT>
        <br>
...
@mysql_free_result($queryexfd);
#print "</table>\n";
#print "<input type=\"hidden\" id=\"eventextfldcnt\" value=\"".$extfldcnt."\">";
print "<!-- end input required extended fields -->\n";
$extreqfldcnt = $extfldcnt;
print "<!-- input standard extended fields -->\n";
#print "<table id=\"extfldtab\"  width=\"50%\" border=\"1\" cellspacing=\"1\" cellpadding=\"1\">\n";
