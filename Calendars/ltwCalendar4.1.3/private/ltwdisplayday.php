...
// $Id: ltwdisplayday.php,v 1.2 2003/08/30 02:38:44 t51admin Exp $
...
var $returnto		= 'month';		// where I came from (list or month)
...
function displayDay($day) {
	...
	$today = $this->year."-".$this->month."-".$day ;

	$query  = "SELECT * FROM ".$this->ctable." ";

	$query .= "WHERE ( (event_date <= '".$today."') AND (event_end >= '".$today."') AND (recurring = 0)) ";

	$query .= "   OR ( (event_date <= '".$today."') AND (event_end >= '".$today."') AND (recurring = 1) ";

	$query .= "         AND (recur_dayofweek = ".date('w',mktime(12,12,12,$this->month,$day,$this->year)).") ) ";

	$query .= "ORDER BY day_event DESC, start_time";
	...
	<html><head>";echo "<title>".$dateformatted."</title></head><body>

	<b>Events for ".$dateformatted."</b>
	...
	if ( $this->auth->checkLogin() && $this->auth->user->getPrivledge(UPEDIT+UPADMIN) ){
	 	...
	 	echo "<br><a href=\"".$this->php_self."?display=admin&amp;task=add&amp;date=" . date("Y-m-d", mktime(12,12,12,$this->month,$day,$this->year)) . "&amp;returnto=".$this->returnto."\">add event</a>\n";
...
// this function is used to 'display' the information about a single event

function displayOneEvent($row=''){
