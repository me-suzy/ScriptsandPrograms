...
<?php
	...
	function display_tasks($userid, $date, $sortby)
	{
		...
		$select_task_query = "select taskid, userid, description, TIME_FORMAT(startdatetime,'%H : %i') as Start_Time , TIME_FORMAT(enddatetime,'%H : %i') as End_Time, priority, iscompleted from calendar_tasks where userid like '$userid' and DATE_FORMAT(startdatetime,'%e-%c-%Y') like '$date' order by $sortby";
